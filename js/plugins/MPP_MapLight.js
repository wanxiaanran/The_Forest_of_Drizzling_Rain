...
(() => {
    ...
    Tilemap.prototype.createDarknessBitmap = function() {
        ...
        const scaleX = this._tileWidth / size;
        const scaleY = this._tileHeight / size;
    ...
    Tilemap.prototype._paintDarknessAllLight = function(layer, sx, sy) {
        for (const light of $gameMap.allMapLights()) {
            layer.paintMapLidht(light, sx, sy);
        }
    };
    ...
    DarknessLayer.prototype.paintMapLidht = function(mapLight, sx, sy) {
        if (mapLight.isValid()) {
            ...
            if (mapLight.isFront()) {
                this._paintFrontLidht(dx, dy, rgb, mapLight.angle, alpha);
            } else {
                this._paintNormalMapLidht(dx, dy, rgb, mapLight.radius, alpha);
            }
        }
    };

    DarknessLayer.prototype._paintFrontLidht = function(x, y, rgb, angle, alpha) {
    ...
    DarknessLayer.prototype._paintNormalMapLidht = function(x, y, rgb, r, alpha) {
        const size = param_DarknessSize;
        const dx = x * size;
        const dy = y * size;
        const dr = r * size;
